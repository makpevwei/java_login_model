...
def login():
    ...
    if request.method=="GET" or request.method=="POST":
        ...
        if  _userId == "user" and _password == "user":
            ...
            app.logger.info('Logging Succesful for',_userId)
            return resp
        
        else:
            
            resp = jsonify('Invalid Username/Password')
            app.logger.info('Logging Failure for',_userId)
            return resp
...
def register():
    ...
    if request.method=="GET" or request.method=="POST":
        ...
        if  _userId == "user" and _mobile == "0803":
            ...
            resp = jsonify(respDict)
            app.logger.error('Logging Succesful for',_userId)
            return resp
        
        else:
            
            resp = jsonify('Invalid Username/Password')
            app.logger.error('Logging Failure for',_userId)
            return resp
